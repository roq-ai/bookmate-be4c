...
type ColumnType = ColumnDef<TimeSlotInterface, unknown>;

interface TimeSlotListPageProps {
...
export function TimeSlotListPage(props: TimeSlotListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TimeSlotInterface>>(
    () => `/time-slots?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/appointments/view/${record.appointment?.id}`}
            >
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/time-slots/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
