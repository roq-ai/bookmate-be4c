...
import { getTimeSlots } from 'apiSdk/time-slots';
...
import { CalendarListPage as ShowCaseList } from 'pages/calendars';

function DashboardPage() {
  const { session } = useSession();

  const timeSlotStat = useSWR<PaginatedInterface<TimeSlotInterface>>(
    () => `/time-slots`,
    () => getTimeSlots({ limit: 1, offset: 0 }),
  );

  const calendarStat = useSWR<PaginatedInterface<CalendarInterface>>(
    () => `/calendars`,
    () => getCalendars({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Time Slots',
      result: timeSlotStat?.data?.totalCount,
      isLoading: timeSlotStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Calendars',
      result: calendarStat?.data?.totalCount,
      isLoading: calendarStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Roles',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`calendars`} passHref legacyBehavior>
                  <Button
