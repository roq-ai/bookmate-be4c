...
type ColumnType = ColumnDef<CalendarInterface, unknown>;

interface CalendarListPageProps {
...
export function CalendarListPage(props: CalendarListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CalendarInterface>>(
    () => `/calendars?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/calendars/view/${record.id}`} passHref legacyBehavior>
                <Button
